...
func ReadWalSummariesForRange(pgDataDir string, timeline uint32,
	...
	state := make(map[relForkKey]*relForkState)
...
func parseWalSummaryFile(path string, state map[relForkKey]*relForkState) error {
	f, err := os.Open(path)
	if err != nil {
		return err
	}
...
func parseSummaryChunks(r io.Reader, nchunks uint32,
	...
	state map[relForkKey]*relForkState) error {
	key := relForkKey{
		rel: walparser.RelFileNode{
	...
	st, ok := state[key]
	if !ok {
		st = &relForkState{blocks: roaring.New()}
		state[key] = st
	}
	...
	if limitBlock != invalidBlockNumber {
		st.blocks.RemoveRange(uint64(limitBlock), 1<<32)
	}
	...
	for chunkNo, used := range usage {
		...
		if err := readChunk(r, st.blocks, uint32(chunkNo), used); err != nil {
			return err
		}
	}
	return nil
